...
class DLGDebugger (object):
    ...
    def save_debugger_state (self):
        s = [ self.dialogs, self.values, self.actions ]
    ...
    def restore_debugger_state (self):
        ...
        s = cPickle.load (fh)
        fh.close ()
        self.dialogs, self.values, self.actions = s
    ...
    def print_conditions (self, cond = None):
        if cond is None:
            ...
            keys.sort ()
        elif self.values.has_key (cond):
            keys = [ cond ]
        else:
            keys = []

        for key in keys:
            print(key, '=', repr (self.values[key]))


    def set_condition_value (self, cond, value = None):
        try:
            oldvalue = self.values[cond]
        except KeyError:
    ...
    def print_state (self, d, state):
        ...
        for transition in d.transition_list[state['first_transition_index']:state['first_transition_index']+state['transition_cnt']]:
            if (transition['flags'] & 0x02) and transition['trigger_index'] != 0xffffffff and not self.eval_trigger (d.transition_trigger_list[transition['trigger_index']]):
                j += 1
                continue
    ...
    def eval_trigger (self, trigger):
        return  self.eval_function (trigger['code'])


    def get_next_state (self, d, state_ndx = 0):
        while True:
            state = d.state_list[state_ndx]
            if state['trigger_index'] == 0xffffffff or self.eval_trigger (d.state_trigger_list[state['trigger_index']]):
                return state_ndx
            else:
                state_ndx += 1
                # FIXME: will it end?
    ...
    def run_dialog (self, name, state = 0):
        ...
        while True:
                ...
                if cmd == 'quit':
                    ...
                    if args is None:
                        ...
                        if value is None:
                            ...
                            self.set_condition_value (cond, value)
            ...
            if address == DLGDebugger.A_GET_STATE:
                state = self.get_next_state (d, state)
                address = DLGDebugger.A_GET_REPLY

            if address == DLGDebugger.A_GET_REPLY:
                self.print_state (d, d.state_list[state])

            if address == DLGDebugger.A_TRANSITION:
                next_dlg, state = self.do_transition (d, d.state_list[state], int (cmd))
                address = DLGDebugger.A_GET_STATE
